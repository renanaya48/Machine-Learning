...
class ConvolutionModel(nn.Module):
    ...
    def __init__(self):
        ...
        """
        self.firstLay = nn.Sequential(
            nn.Conv2d(1, 16, kernel_size=5, stride=1, padding=2),
            nn.ReLU(),
            nn.MaxPool2d(kernel_size=2, stride=2))
        self.secondLay = nn.Sequential(
            nn.Conv2d(16, 64, kernel_size=5, stride=1, padding=2),
            nn.ReLU(),
            nn.MaxPool2d(kernel_size=2, stride=2))
        self.thirdLay = nn.Sequential(
            nn.Conv2d(64, 15, kernel_size=5, stride=1, padding=2),
            nn.ReLU(),
            nn.MaxPool2d(kernel_size=2, stride=2))
            """
        # build drop out
        ...
        for i, (examples, tags) in enumerate(train_loader):
            ...
            if (i + 1) % 100 == 0:
                print('Epoch [{}/{}], Step [{}/{}], Loss: {:.4f}, Accuracy: {:.2f}%'
                      .format(epoch + 1, epochs, i + 1, train_length, loss.item(),
                              (correct_predicts / all_predicts) * 100))
...
def validate(model, valid_loader):
    ...
    with torch.no_grad():
        ...
        for examples, tags in valid_loader:
            ...
            correct_predicts += (predicted == tags).sum().item()
        print(
            'Test Accuracy of the model on the 10000 test images: {} %'.format((correct_predicts / all_predicts) * 100))
