...
def loss_function(clust, norm, centers):
    counter = 0
    sum = 0
    len=len(clust)

    for line in range(len):
        for point in range(len(clust[line])):
            d = 0
            dis = None
            for center in range(len(centers)):
                d = sp.distance.euclidean(norm[line][point], centers[center])
                if (dis == None):
                    dis = d
                elif(d < dis):
                        dis = d
    ...
    avg = sum/counter
    ans = np.linalg.norm(avg)
    print(ans)
    return

def print_cent(center):
    if type(center) == list:
        center = np.asarray(center)
    if len(center.shape) != 1:
        toPrint= ' '.join(str(np.floor(100 * center) / 100).split()).replace('[ ', '[').replace('\n', ' ').replace(' ]',']').replace(' ', ', ')[1:-1]
    else:
        toPrint= ' '.join(str(np.floor(100 * center) / 100).split()).replace('[ ', '[').replace('\n', ' ').replace(' ]',']').replace(' ', ', ')
    print(toPrint)
...
def graph(it, loss):
    #loss = [1,2,1,4,5,8,1,3,4,7]
    plt.plot(loss)
    plt.title('Average loss, K = %d' %it)
    plt.ylabel('Loss')
...
def center_avg(clust, norm, k, rostCenter):
    green = 0
    red = 0
    blue = 0
    counter = 0
    ans = [None] * 3
    for line in range(len(clust)):
        for col in range(len(clust[line])):
            if (clust[line][col] == k):
                counter += 1
                red += norm[line][col][0]
                green += norm[line][col][1]
                blue += norm[line][col][2]
        #end of columns
    #end of lines
    if (counter == 0):
        return rostCenter
